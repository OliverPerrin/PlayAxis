...
async def get_sportsbook_events(sport: str):
    try:
        ...
        key = f"sportsbook:{sport_param}"

        async def producer():
            async with httpx.AsyncClient(timeout=30.0) as client:
                r = await client.get(url, headers=headers, params=params)
                if r.status_code == 429:
                    logger.error(f"Sportsbook 429 sport={sport_param} body={r.text[:300]}")
                    # brief backoff to avoid thundering herd; return latest cache if available
                    await asyncio.sleep(0.5)
                    return []
                if r.status_code == 403:
                    logger.error(f"Sportsbook 403 sport={sport_param} body={r.text[:300]}")
                    return []
        ...
        if cached is not None:
            return cached
        out = await producer()
        await cache.set(key, out, SPORTSBOOK_TTL)
        return out
